...
Working on `graph/graphify.py` (the Excel/Word/Google-Workspace secondary engine) needs its own extra, separate from `dev`:
...
- **Subprocess-spawning code gets a fake executable on `PATH`, not a mocked `subprocess.Popen`**, wherever a real (if trivial) process is feasible — see `tests/orchestrator/conftest.py`'s `fake_claude_bin` fixture, used by `test_adapter.py` and `test_claude_code.py`.
...
- **Subprocess calls are always an argv list (`shell=False`), never a shell string**, and are scoped to a per-module subcommand allowlist where the argv is partly caller-influenced (see `orchestrator/worktree.py`'s `_GIT_LIFECYCLE_SUBCOMMANDS`). josu doesn't spawn gortex as a long-lived service at all -- the one remaining gortex-related subprocess call is a short-lived, best-effort `gortex version` compatibility check, and it still gets an explicit, minimal environment (`PATH`/`HOME` only) rather than inheriting the daemon's full environment (which carries resolved delegate API credentials) — see `graph/gortex_process.py`'s `_minimal_subprocess_env()`.
