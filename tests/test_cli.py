"""Tests for the `josu` CLI's argparse `--help` surface (`src/josu/cli.py`),
per docs/plans/2026-08-06-001-fix-cli-ease-of-use-plan.md (origin:
...
`_cmd_run`'s config-warning-printing behavior is covered in
`tests/test_cli_run.py`, which already establishes the `fake_daemon` real-
TCP-listener convention for testing this exact call site -- see that file's
