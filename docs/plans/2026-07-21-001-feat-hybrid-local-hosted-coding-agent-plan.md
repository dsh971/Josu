...
- **Approach:** `engine.py` defines the graph-engine interface (build, query) that `build.py`'s graphify wrapper implements, kept behind this interface so swapping the engine later doesn't touch `server.py` (R10). `server.py` uses `mcp.server.lowlevel.Server` with `list_tools()` returning exactly two static schemas (`search`, `execute`) and `call_tool()` dispatching internally on operation name, validating arguments with per-operation Pydantic models rather than exposing that structure to the client (R12). Pin `mcp` to v1.x. `daemon.py` mounts this server over a local HTTP/SSE transport (not stdio) so it's one long-running process reachable by every worktree's Claude Code invocation and every direct-call path — see Key Technical Decisions.
...
- **Approach:** **Before implementing, verify graphify supports a scoped/partial build** — U1's engine interface is currently `build`/`query` only; if graphify has no incremental path, extend the interface before this unit depends on it. Once verified, content-hash change detection scopes re-indexing to changed files, triggered by the same commit/save events as U9 plus when a merge (U5) completes — a bounded, already-known set of changed files.
- **Patterns to follow:** claude-context's Merkle-tree incremental indexing (see origin Sources/Research).
- **Test scenarios:**
  - Graphify's incremental-build capability check passes before any re-index logic runs.
